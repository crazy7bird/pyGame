...
class farmer():
    def __init__(self,x : int) -> None:
        ...
        for i in range(N_IMG_MOVING) :
            ...
            self.backwardImg[i].width -= X_SIZE
        self.stillAnim = pyglet.image.Animation.from_image_sequence(self.stillImg, duration=0.5, loop=True)
        self.forwardAnim = pyglet.image.Animation.from_image_sequence(self.forwardImg, duration=0.5, loop=True)
        self.backwardAnim = pyglet.image.Animation.from_image_sequence(self.backwardImg, duration=0.5, loop = True)
